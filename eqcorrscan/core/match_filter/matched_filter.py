...
def match_filter(
    ...
    **kwargs,
    ...
    if arg_check:
        ...
        for template in template_list:
            if not isinstance(template, Stream):
                msg = (
                    "template in template_list must be of type: "
        ...
        for template in template_list:
            for tr in template:
                if not tr.stats.sampling_rate == st[0].stats.sampling_rate:
                    raise MatchFilterError(
                        "Template sampling rate does not " "match continuous data"
                    )
        for template in template_list:
            for tr in template:
    ...
    name_mapper = {
        template_name: f"template_{i}" for i, template_name in enumerate(template_names)
    }
    for template, template_name in zip(template_list, template_names):
        tribe += Template(
            st=template,
            name=name_mapper[template_name],
    ...
    Logger.info(
        "Made {0} detections from {1} templates".format(len(detections), len(tribe))
    )
